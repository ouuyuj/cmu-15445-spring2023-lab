...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  std::shared_ptr<StringExpression> expression;

  if (args.size() == 1) {
    AbstractExpressionRef arg = args[0];
    if (func_name == "upper") {
      expression = std::make_shared<StringExpression>(arg, StringExpressionType::Upper);
    } else if (func_name == "lower") {
      expression = std::make_shared<StringExpression>(arg, StringExpressionType::Lower);
    } else {
      throw Exception(fmt::format("func call {} not supported in planner yet", func_name));
    }
  } else {
    throw Exception("statement error\n");
  }
