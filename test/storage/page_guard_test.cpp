...
TEST(PageGuardTest, WriteTest2) {
  ...
  // test ReadPageGuard::operator=(ReadPageGuard &&that)
  {
    auto writer_guard_1 = bpm->FetchPageWrite(page_id_temp);
    auto writer_guard_2 = bpm->FetchPageWrite(page_id_temp);
    EXPECT_EQ(2, page0->GetPinCount());
    writer_guard_1 = std::move(writer_guard_2);
    EXPECT_EQ(1, page0->GetPinCount());
  }
  EXPECT_EQ(1, page0->GetPinCount());
